...
class Ball():
    def __init__(self, x, y, colour, radius):
        ...
        self.ballOutline=pygame.draw.circle(screen, BLACK, (self.x, self.y), self.radius, 1)
        self.ballRect=pygame.Rect(self.x, self.y, self.radius*2, self.radius*2)
        self.ballOutlineRect=pygame.Rect(self.x, self.y, self.radius*2, self.radius*2)
...
class Fog(pygame.sprite.Sprite):
    def __init__(self, x, y):
        ...
        #creating a bunch of images so that size change might run faster
        self.images=[]
        self.widths=[]
        self.heights=[]
        self.imageNum=0
        ...
        for i in range(0,100):
            self.images.append(pygame.transform.scale(self.originalImage, (self.width, self.height)))
    ...
    def move(self, x, y):
        self.rect.x=x-self.widths[self.imageNum]//2
        self.rect.y=y-self.heights[self.imageNum]//2
        #dealing with size
        if self.imageNum!=0 and self.imageNum!=self.oldImageNum and self.imageNum<len(self.images):
            self.image=self.images[self.imageNum]
            self.oldImageNum=self.imageNum
        
    def changeSize(self):
        self.width+=10
        if self.width in self.widths and self.imageNum!=len(self.images)-1:
            self.imageNum+=1

    def unchangeSize(self):
        if self.imageNum!=0:
            self.width-=10
            if self.width in self.widths:
                self.imageNum-=1
...
class PlayingState:
    def __init__(self, maze):
        ...
        #trailing ball stuff
        self.trails=[]
        self.colours=[RED, ORANGE, YELLOW, DARK_GREEN, LIGHT_BLUE, BLUE, PURPLE, MAGENTA]
        self.frame=0
    ...
    def updateTimers(self):
        ...
        if self.maze==1:
            ...
            if self.time<self.fastest:
                self.fastest=self.time
            print(self.fastest, self.time, "Actual", self.maze1Fastest+self.maze2Fastest+self.maze3Fastest)
            replaceTimes(self)
...
def replaceTimes(stats:PlayingState):
    ...
    if checkIfFastest(stats.maze3Time, "maze3TopTime", stats.level):
        ...
        stats.maze3New=True
    print(stats.fastest)
    if checkIfFastest(stats.fastest, "totalTopTime", stats.level):
        pushToFile(str(stats.fastest), "totalTopTime", stats.level)
        stats.totalNew=True
...
def showBestTimes(stats:PlayingState):
    ...
    if stats.maze1New:
        toScreen("New Fastest Time!", font30, PURPLE, 175, HEIGHT//2+35)
    if stats.maze2New:
        toScreen("New Fastest Time!", font30, PURPLE, WIDTH//2, HEIGHT//2+25)
    if stats.maze3New:
        toScreen("New Fastest Time!", font30, PURPLE, WIDTH-210, HEIGHT//2+35)
    if stats.totalNew:
        toScreen("New Fastest Time!", font30, PURPLE, WIDTH//2, HEIGHT//2+270)
...
def intro(gameState):
    global running
    #putting into groups
    button=Button(WIDTH//2-75, HEIGHT//2+85, 150, 100)
    ...
    toScreen3("There will be multiple choices for the difficulty level.", "Fog mean you can only see part of the maze, if you don't move, that area will temperarily increase", "Teleporters will teleport you to another teleporter if you click space while touching one.", font15, WHITE, WIDTH//2, HEIGHT//2+50)
...
def playing(playingStuff: PlayingState):  
    ...
    playingStuff.trails.append(Ball(playingStuff.player.x+playingStuff.player.width//2, playingStuff.player.y+playingStuff.player.height//2, playingStuff.colours[playingStuff.frame%len(playingStuff.colours)], 5))
    for ball in playingStuff.trails:
        ball.update()
        ball.draw()
    playingStuff.frame+=1
    ...
    #going back to the end screen if they have returned
        ...
        print(playingStuff.fastest, playingStuff.time, "Actual", playingStuff.maze1Fastest+playingStuff.maze2Fastest+playingStuff.maze3Fastest)
